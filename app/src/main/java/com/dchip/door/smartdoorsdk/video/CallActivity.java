...
import com.dchip.door.smartdoorsdk.Bean.videoinfo;
            ...
            case MSG_CALL_MAKE_VOICE:
                try {
                    if (msg.what == MSG_CALL_MAKE_VIDEO) {
                        ...
                        v.setLockID(DPDB.getLockID()+"");
                        v.setUid(DPDB.getmac()+"lockId");
                        v.setAddr(DPDB.getaddr());
                        EMClient.getInstance().callManager().makeVideoCall(username, GsonUtil.getIns().beanToJSONString(v));
                    } else { 
